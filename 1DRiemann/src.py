...
class PINN(nn.Module):
    def __init__(self, model, model2, device='cpu', Xmin=0.0, Xmax=1.0, Tmin=0.0, Tmax=0.2):
        super().__init__()
        self.model = model
        self.model2 = model2
        self.device = device
    ...
    def train(self, x_physics, x_boundary, u_boundary,epochs=501, batch_size=8192):
        ...
        for epoch in range(epochs):
            ...
            if epoch % 100==0:
                ...
                x = torch.arange(self.Xmin, self.Xmax + step_size_space, step_size_space)
                y = torch.arange(self.Tmin, self.Tmax + step_size_time, step_size_time)
                ...
                grid = grid.clone().detach().float().to(self.device)
