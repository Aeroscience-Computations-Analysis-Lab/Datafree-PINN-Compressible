...
class FNN(nn.Module):
    def __init__(self, inputs, layers, init_type='xavier', output2='exp'):
        ...
        # Normalize inputs
        inputs_tensor = inputs.clone().detach().float()
...
class PINN(nn.Module):
    def __init__(self, model, model2, device='cpu', Xmin=0.0, Xmax=1.0, Ymin=0.0, Ymax=1.0, Tmin=0.0, Tmax=1.0):
        super().__init__()
        self.model = model
        self.model2 = model2
        self.device = device
    ...
    def train(self, x_train, x_initial, ic, epochs=1000, batch_size=10000):
        ...
        for epoch in range(epochs):
            ...
            if epoch % 1000 == 0:
                ...
                step_size = 0.01
                x = torch.arange(self.Xmin, self.Xmax + step_size, step_size)
                y = torch.arange(self.Ymin, self.Ymax + step_size, step_size)
                t = torch.arange(self.Tmin, self.Tmax + step_size, step_size)
